...
## 2024-03-18 - Search Inputs Need Explicit ARIA Labels
**Learning:** Text inputs that rely solely on visual `placeholder` text (like search bars) do not reliably announce their purpose to screen readers.
**Action:** Always provide an explicit, descriptive `aria-label` for search and text inputs. When updating `placeholder` dynamically via JavaScript, update `aria-label` simultaneously to keep screen readers synchronized.
