...
### ⚠️ NEW VIOLATIONS (2026-03-23 scheduled QA)
- **[DS-01] `font-style:italic` — 2 sites** — `js/quiz.js:471` (swap narrative inline style) + `js/quiz.js:740` (archetype tagline embedded CSS). DESIGN.md no-italics rule added this branch. Fix: remove italic, remove hardcoded `#B0A898` hex fallback on line 471.
- **[DS-02] Canvas italic** — `js/app.js:923` uses `italic 36px Georgia` for recipe card PNG export. Low priority (canvas output); still a violation of font conventions.

### Won't fix for now
- **[DS-03] Score meter full rename** — `.cmp-score-pct` → `.score-display` in JS call sites (30+ sites). CSS aliases added; full rename deferred until Compare panel is refactored.
- **[DS-04] Dupe breakdown class rename** — `.dupe-breakdown` → `.callout` structure change. Current CSS is correct; structural rename risks visual regression. Deferred.
- **[DS-05] `.cmp-card-detail-btn`** — Still in use in Compare panel. Migrate to `.link-btn.link-btn--primary` when Compare panel is refactored.
...
4. **[P0-04]** Call `engine.getSwapReason(top3[0], top3[1], FAM)` in `renderResult()` when `top3.length >= 2` — show 1-sentence narrative between subtitle and first card; apply to **both** `app.js` and `quiz.js renderResults()`
5. **[P0-05]** Add `history.replaceState(null,'','?results=' + top3.map(f=>f.id).join(','))` on result render in `app.js`; restore from `?results=` param on load (quiz.js already handles this)
6. **[P0-06]** Wrap all `sessionStorage` reads/writes in try/catch (iOS private mode throws)
...
9. **[P0-09]** Wire `trackEvent()` to Supabase inserts in **both** `app.js` and `quiz.js` — `if (!_sb) return;` null-guard required; `.catch(e => console.warn('[analytics]', e))` on each insert. **Gate:** confirm `events` table exists first (schema: `id uuid pk, name text, props jsonb, ts timestamptz default now()`, anon INSERT RLS enabled)
10. **[P0-10]** Add "Try a sample →" CTA to `quiz.js renderResults()` when `config.scoring?.giftMode` is true — fire `sample_link_click` event; success gate depends on this path
...
### TODO: [P2-XS-01] Catalog Panel Access from app.html Nav
**What:** `app.html` contains `#p-catalog` but no nav item activates it. The "Discovery" link goes to `/#discovery` (landing page). Add catalog access: either (a) change "Discovery" desktop+mobile nav to `goMobile('catalog', this)` and `go('catalog', this)`, OR (b) decide Discovery always links away and document this as intentional.
**Why:** Users who open `/app.html` directly have no way to browse/search the catalog via primary navigation — they must use the global search bar. QA ISSUE-003 (2026-03-23).
**Effort:** XS (2-line HTML change, or a product decision)

---

### TODO: [P2-XS-02] Brand Card Hover Affordance
...
### TODO: [P2-XS-03] Gift-mode-aware heading in quiz.js renderResults()
...
### TODO: [P2-XS-04] DNA Card Profile Bars — ARIA
...
### TODO: [P2-XS-05] Golden Pairs Carousel — Keyboard Nav
...
### TODO: [P2-XS-06] State Bar Collection Count Text
...
### TODO: [P2-XS-07] Carousel Focus Restoration After Detail Close
...
### TODO: [P2-S-01] Smart Wishlist Priority ("Next Buy" Flag)
...
### TODO: [P2-S-02] "More Like This" Diversity Boost
...
### TODO: [P2-S-03] Zero-Owned State — Onboarding Prompt
...
### TODO: [P2-S-04] Carousel Prev/Next Buttons
...
### TODO: [P2-S-05] Playwright test — standalone gift quiz flow
...
### TODO: [P2-S-06] Playwright test — dupe page
...
### TODO: [P2-S-07] Extract note-diff logic into engine.js
...
### TODO: [P2-M-01] List Item Component Consolidation
...
### TODO: [P2-M-02] Sillage & Layering Data Quality Audit
...
#### TODO: [SR1-01] Brand Positioning & Manifesto
...
#### TODO: [SR1-02] Custom Supabase Event Analytics
...
#### TODO: [SR2-01] Trust Architecture — "Show Your Math"
...
#### TODO: [SR2-02] Compare Stories — "Why This, Not That"
...
#### TODO: [SR2-03] Zero-State First Visit Experience
...
#### TODO: [SR3-01] Brand Gift Guide Pages
...
#### TODO: [SR3-02] Individual Fragrance Pages
...
#### TODO: [SR4-01] Sampling Partnership Infrastructure
...
### TODO: [DEF-01] Delete gift-intelligence from quiz-config.json
...
| ID | Item | What | Effort | Blocker |
|---|---|---|---|---|
| **[P3-01]** | **Scent Persona** | Shareable identity card from collection; 16 archetypes from profile axes | M | Share card layout |
| **[P3-02]** | **Scentscape** | Daily 3-tap mood picker recommends from owned collection | M | — |
| **[P3-03]** | **Season Wardrobe** | Map owned frags to current season via family + sillage scoring | M | — |
| **[P3-04]** | **Zodiac Quiz Bridge** | Upgrade `/quiz/astro-scent`; Add to Collection button seeds app state | M | — |
| **[P3-05]** | **Big Three Astro** | Sun + Moon + Rising composite match; 1,728 combinations | M | 36 copy lines |
| **[P3-06]** | **Layering Lab** | Pick 2 frags → compatibility %, combined radar, recipe card | M | — |
| **[P3-07]** | **Compare → Gift Card** | Gift-framing share output from Compare results | M | Visual spec |
| **[P3-08]** | **Compare Keyboard Shortcuts** | `X` to swap, `Backspace` to clear; static hint line; aria-live announce | S | — |
| **[P3-09]** | **Note Glossary Tooltips** | 1-sentence description on note name hover | M | 177 note descriptions |
| **[P3-10]** | **Fragrance Memory Journal** | ≤140-char memory note per frag; occasion/person/season tags | M | — |

---

## Infrastructure

### TODO: [INFRA-01] Scent Data Consolidation
