...
// Validates data/scents.json against known roles, families, and notes.
...
const frags  = JSON.parse(fs.readFileSync(path.join(DATA, 'scents.json'), 'utf8'));
...
if (!Array.isArray(frags)) {
  console.error('ERROR: data/scents.json must be a JSON array');
  process.exit(1);
}
...
console.log(`  ✓ scents.json — ${frags.length} fragrances`);
console.log('');
if (errors) {
