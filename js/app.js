...
function openMoreSheet(btn){
  ...
  pushSheet(el=>{
    el.innerHTML=`<div style="padding:16px 0 8px">
      ...
      ${items.map(it=>`
        <button onclick="closeAllSheets();goMobile('${it.id}',document.querySelector('.mbn-more'))" style="display:flex;align-items:center;gap:14px;width:100%;background:none;border:none;padding:14px 16px;cursor:pointer;font-family:inherit;font-size:.88rem;color:var(--text-primary);border-bottom:1px solid var(--border-standard);text-align:left">
          <span style="font-size:1.2rem;width:24px;text-align:center;flex-shrink:0">${it.icon}</span>
          ${it.label}
        </button>`).join('')}
        <a href="playground.html" style="display:flex;align-items:center;gap:14px;width:100%;background:none;border:none;padding:14px 16px;cursor:pointer;font-family:inherit;font-size:.88rem;color:var(--text-primary);border-bottom:1px solid var(--border-standard);text-align:left;text-decoration:none;">
          <span style="font-size:1.2rem;width:24px;text-align:center;flex-shrink:0">🎨</span>
          Design System
        </a>
    </div>`;
  });
