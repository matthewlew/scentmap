...
function buildLayerSuggestions(frag,container){
  ...
  const candidates=owned
    .map(f=>{
      ...
      return {f,score:scoreObj.total, scoreObj};
    })
    .filter(x=>x.score>=40)
    .sort((a,b)=>b.score-a.score)
    .slice(0,2);
  ...
  container.appendChild(lbl);
  const shelf=document.createElement('div');shelf.className='list-view';
  candidates.forEach(({f,score,scoreObj})=>{
    const fm2=FAM[f.family]||{color:'#888'};
    ...
    row.innerHTML=`
        ...
        <div class="list-item-body">
          ...
          ${reason ? `<div class="list-item-detail text-caption">${reason}</div>` : ''}
          <div class="list-item-detail text-caption" style="color:var(--text-tertiary);margin-top:2px;">Math: Family +${scoreObj.famScore} | Sillage +${scoreObj.sillScore} | Notes +${scoreObj.noteScore}</div>
        </div>
        <div class="list-item-trail">
