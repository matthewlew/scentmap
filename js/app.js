...
function buildLayerSuggestions(frag,container){
  ...
  const candidates=owned
    .map(f=>{ const s = scoreLayeringPair(frag,f); return {f, score: s.total, breakdown: s}; })
    .filter(x=>x.score>=40)
    .sort((a,b)=>b.score-a.score)
    .slice(0,2);
  if(!candidates.length)return;
  ...
  candidates.forEach(({f,score,breakdown})=>{
    ...
    row.innerHTML=`
        ...
        <div class="list-item-body">
          ...
          ${reason ? `<div class="list-item-detail text-caption">${reason}</div>` : ''}
          <div class="list-item-detail text-caption" style="margin-top:2px; color:var(--text-tertiary);">Math: Fam +${breakdown.famScore} | Sill +${breakdown.sillScore} | Note +${breakdown.noteScore}</div>
        </div>
        <div class="list-item-trail">
