...
function buildLayerSuggestions(frag,container){
  ...
  const candidates=owned
    .map(f=>({f,breakdown:scoreLayeringPair(frag,f)}))
    .filter(x=>x.breakdown.total>=40)
  ...
  container.appendChild(lbl);
  const shelf=document.createElement('div');shelf.className='list-view';
  candidates.forEach(({f,breakdown})=>{
    const fm2=FAM[f.family]||{color:'#888'};
    ...
    row.innerHTML=`
        ...
        <div class="list-item-body">
          ...
          ${reason ? `<div class="list-item-detail text-caption">${reason}</div>` : ''}
          <div class="list-item-detail text-caption">Math (Fam: ${Math.round(breakdown.famScore)}, Sill: ${Math.round(breakdown.sillScore)}, Notes: ${Math.round(breakdown.noteScore)})</div>
        </div>
        <div class="list-item-trail">
          <span class="dc-layer-score-badge">${breakdown.total}</span>
