...
function buildLayerSuggestions(frag,container){
  ...
  const candidates=owned
    .map(f=>{
      const lay = scoreLayeringPair(frag, f);
      return { f, score: lay.total, lay };
    })
    .filter(x=>x.score>=40)
    .sort((a,b)=>b.score-a.score)
    .slice(0,2);
  ...
  container.appendChild(lbl);
  const shelf=document.createElement('div');shelf.className='list-view';
  candidates.forEach(({f,score,lay})=>{
    const fm2=FAM[f.family]||{color:'#888'};
    const reason=layerReason(frag,f);
    const breakdown = `Family: ${lay.famScore} | Sillage: ${lay.sillScore} | Notes: ${lay.noteScore}`;
    const row=document.createElement('button');
    row.className='list-item';
    row.innerHTML=`
        <div class="list-item-dot" style="--fam-bg: ${fm2.color}"></div>
        <div class="list-item-body">
          ...
          ${reason ? `<div class="list-item-detail text-caption">${reason} (${breakdown})</div>` : `<div class="list-item-detail text-caption">${breakdown}</div>`}
        </div>
        <div class="list-item-trail">
