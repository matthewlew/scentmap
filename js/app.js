  ...
  for (let i = 0; i < owned.length; i++) {
    for (let j = i + 1; j < owned.length; j++) {
      ...
      if (scoreObj.total >= 50) {
        pairs.push({ a: owned[i], b: owned[j], score: Math.round(scoreObj.total / 75 * 100) });
      }
    }
  }
...
function buildLayerSuggestions(frag,container){
  ...
  const candidates=owned
    .map(f=>({f,scoreObj:scoreLayeringPair(frag,f)}))
    .filter(x=>x.scoreObj.total>=40)
  ...
  candidates.forEach(({f,scoreObj})=>{
    ...
    row.innerHTML=`
        ...
        <div class="list-item-body">
          ...
          ${reason ? `<div class="list-item-detail text-caption">${reason} &bull; Math: Fam +${scoreObj.famScore}, Sill +${scoreObj.sillScore}, Note +${scoreObj.noteScore}</div>` : ''}
        </div>
        <div class="list-item-trail">
