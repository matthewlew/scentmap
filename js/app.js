  ...
  for (let i = 0; i < owned.length; i++) {
    for (let j = i + 1; j < owned.length; j++) {
      ...
      if (scoreObj.total >= 50) {
        pairs.push({ a: owned[i], b: owned[j], scoreObj: scoreObj, score: Math.round(scoreObj.total / 75 * 100) });
      }
    }
  }
    ...
    if (pairs.length > 0) {
      ...
      pairs.forEach(p => {
        ...
        card.innerHTML = `
          ...
          <div class="text-meta" style="margin-top:var(--sp-sm); padding-top:var(--sp-xs); border-top:1px solid var(--border-light); color:var(--text-secondary); display:flex; gap:var(--sp-sm); justify-content:space-between;">
            <div>Family: ${Math.round((p.scoreObj.famScore / 35) * 100)}%</div>
            <div>Sillage Contrast: ${Math.round((p.scoreObj.sillScore / 20) * 100)}%</div>
            <div>Note Overlap: ${Math.round((p.scoreObj.noteScore / 20) * 100)}%</div>
          </div>
...
function buildLayerSuggestions(frag,container){
  ...
  const candidates=owned
    .map(f=>({f,score:scoreLayeringPair(frag,f).total}))
    .filter(x=>x.score>=40)
    .sort((a,b)=>b.score-a.score)
    .slice(0,2);
  if(!candidates.length)return;
