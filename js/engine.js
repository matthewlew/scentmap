...
export function getSimilarityDetails(a, b, FAM_COMPAT) {
  if (a.id === b.id) return { score: 0, famScore: 0, noteScore: 0, sillScore: 0, roleScore: 0 };
  ...
  const score = Math.round(famScore + noteScore + sillScore + roleScore);
  return { score, famScore: Math.round(famScore), noteScore: Math.round(noteScore), sillScore: Math.round(sillScore), roleScore: Math.round(roleScore) };
}
...
export function scoreSimilarity(a, b, FAM_COMPAT) {
  return getSimilarityDetails(a, b, FAM_COMPAT).score;
}
...
export function getLayeringDetails(a, b, FAM_COMPAT) {
  ...
  const score = Math.round(famScore + sillScore + noteScore);
  return { score, famScore: Math.round(famScore), sillScore: Math.round(sillScore), noteScore: Math.round(noteScore) };
}
...
export function scoreLayeringPair(a, b, FAM_COMPAT) {
  return getLayeringDetails(a, b, FAM_COMPAT).score;
}
