...
/**
 * Calculates and returns the detailed components of the similarity score.
 */
export function getSimilarityDetails(a, b, FAM_COMPAT) {
  if (a.id === b.id) return { famScore: 0, noteScore: 0, sillScore: 0, roleScore: 0, total: 0, famComp: 0, shBase: 0, shMid: 0, shTop: 0, sillDiff: 0, shRoles: 0 };
  const famComp = FAM_COMPAT[a.family]?.[b.family] ?? 0.5;
  const famScore = famComp * 40;
  ...
  return { famScore, noteScore, sillScore, roleScore, total, famComp, shBase, shMid, shTop, sillDiff, shRoles };
}
...
/**
 * Calculates and returns the detailed components of the layering compatibility score.
 */
export function getLayeringDetails(a, b, FAM_COMPAT) {
  ...
  return { famScore, sillScore, noteScore, total, famComp, sillDiff, shared };
}
