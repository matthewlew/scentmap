...
Scentmap follows classic typography principles for copy and data files (e.g., JSON descriptions, tooltips, UI text). Always adhere to these rules when writing or modifying text content:

- **Quotes:** Use curly quotes (“ ” and ‘ ’) instead of straight "dumb" quotes (" and ').
- **Spacing:** Use exactly one space between sentences, never two.
- **Apostrophes:** Use proper typographic apostrophes (’) rather than straight foot marks (').
- **Dashes:**
  - Use an **em dash** (—) without surrounding spaces for a break in thought (e.g., "The classic — and still the best").
  - Use an **en dash** (–) to indicate a range (e.g., "10–20 ml").
  - Use a **hyphen** (-) only for compound words (e.g., "high-quality").
- **Symbols:** Use true typographic symbols where appropriate, such as an ellipsis (…) instead of three periods (...), or a multiplication sign (×) instead of the letter 'x'.
- **Scale:** The CSS typography scale is based on an approximate Major Third (1.250) ratio. Rely on `--fs-*` and `--lh-*` CSS variables instead of hardcoded values to preserve this harmony.
